...
<section class="ftco-section">
    <div class="container">
        <div class="card shadow-sm">
            ...
            <div class="card-body">
                <h2 class="fw-bold" style="font-family: Abril Fatface, serif;"><?= esc($tour['name']) ?></h2>
                <div class="mb-3">
                    <div>
                        <?php if (!empty($tour['classification_names'])) : ?>
                            <?php foreach (explode(',', $tour['classification_names']) as $classification) : ?>
                                <span class="badge bg-primary rounded text-white" style="font-size: 20px;"><?= esc(trim($classification)) ?></span>
                            <?php endforeach; ?>
                        <?php else : ?>
                    ...
                    <div><i class="fas fa-check fa-lg"></i>
                        <?php if (!empty($tour['category_names'])) : ?>
                            <?php foreach (explode(',', $tour['category_names']) as $category) : ?>
                                <span class="badge bg-secondary rounded text-white" style="font-size: 20px;"><?= esc(trim($category)) ?></span>
                            <?php endforeach; ?>
                        <?php else : ?>
                ...
                <a href="<?= base_url('/destination') ?>" class="btn btn-secondary">
