                ...
                <div class="col-md-4 ftco-animate">
                    <div class="project-wrap">
                        ...
                        <div class="text p-4">
                            ...
                            <div class="d-flex">
                                <form action="<?= base_url('/transactions/add') ?>" method="post" style="margin: 2px;" novalidate>
                                    <?= csrf_field() ?>
                                    <input type="hidden" name="tour_id" value="<?= esc($tour['id']) ?>">
                                    <input type="hidden" name="customer_id" value="<?= esc(session()->get('customer_id')) ?>">
                                    <?php if (session()->get('logged_in')): ?>
                                        <button type="submit" class="btn btn-primary">
                                            Add to List <i class="fas fa-plus"></i>
                                        </button>
                                    <?php else: ?>
                                        <a href="/logins" class="btn btn-secondary text-white">
                                            Login
                                        </a>
                                    <?php endif; ?>
                                </form>
