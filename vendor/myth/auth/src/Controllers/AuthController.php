...
class AuthController extends Controller
{
    ...
    public function attemptLogin()
    {
        ...
        if ($user->role === 'admin') {
            $redirectURL = site_url('admins-index'); // Ubah ke halaman admin
        } elseif ($user->role === 'customer') {
            $redirectURL = site_url('/'); // Contoh redirect ke halaman customer
        } else {
            $redirectURL = session('redirect_url') ?? site_url('/');
        }
    ...
    public function attemptRegister()
    {
        ...
        $rules = config('Validation')->registrationRules ?? [
            ...
            'telphone' => 'required|numeric|min_length[10]',
        ...
        // Save the user
        $allowedPostFields = array_merge(['password', 'telphone'], $this->config->validFields, $this->config->personalFields);
        ...
        if ($image && $image->isValid() && ! $image->hasMoved()) {
            ...
            $user->image = $imageName; // Simpan nama file ke kolom `image` jika perlu
        }
    ...
    public function attemptForgot()
    {
        ...
        return redirect()->route('forgot')->with('success', 'Link Reset Password Berhasil Dikirim ke Email Anda');
    }
